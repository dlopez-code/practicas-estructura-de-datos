...
void Digrafo::limpiar(){
    totalNodos = 0;
    totalAristas = 0;
    totalConexiones = 0;
}

bool Digrafo::existeNodo(string nodo){
...
bool Digrafo::existeArista(string arista){
...
int Digrafo::indiceNodo(string nodo){
    for(int i = 0; i < totalNodos; i++){
        if(nodos[i] == nodo){
            return i;
        }
...
string Digrafo::obtenerTexto(string linea){
...
int Digrafo::obtenerNumero(string linea){
    int dosPuntos = linea.find(":");
    string numero = "";
...
void Digrafo::cargarLista(string linea, string arreglo[], int &total){
    int posicion = 0;
...
void Digrafo::cargarJSON(){
    limpiar();

    ifstream archivo("digrafo.json");

    if(!archivo){
        cout << "No se pudo abrir digrafo.json" << endl;
        return;
    }
    ...
    archivo.close();
    cout << "Digrafo cargado correctamente desde digrafo.json" << endl;
}

void Digrafo::mostrarDigrafo(){
    cout << endl;
    cout << "NODOS:" << endl;
    ...
    cout << endl;
    cout << "CONEXIONES DEL DIGRAFO:" << endl;
...
void Digrafo::guardarJSON(){
    ...
    archivo.close();
    cout << "Archivo digrafo.json guardado correctamente." << endl;
}

void Digrafo::mejorRuta(int tipo){
    string inicio, fin;
    cout << "Nodo inicial: ";
    cin >> inicio;
    cout << "Nodo final: ";
    ...
    for(int vuelta = 0; vuelta < totalNodos; vuelta++){
        ...
        for(int i = 0; i < totalConexiones; i++){
            if(conexiones[i].nodoInicial == nodos[actual]){
                int vecino = indiceNodo(conexiones[i].nodoFinal);
                int peso;
